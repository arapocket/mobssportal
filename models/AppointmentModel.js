...
module.exports.getAppointment = function (orderID, callback) {
...
module.exports.deleteAppointment = function (orderID, callback){
    db.createConnection(function (err, result){
        if (err){
            console.log('Error connecting to db');
            callback(err, null);
            ...
            var query = 'DELETE FROM appointment WHERE AppointmentID = "' + orderID + '";'
            console.log(query);
            connection.query(query, function (err, rows, fields) {
                if (!err) {
                    connection.end();
...
module.exports.postAppointment = function (user, Body, callback) {
    ...
    db.createConnection(function (err, result) {
        if (err) {
            ...
            var queryValues = '"' + user + '", "' + orderID + '", "' + Body.Subject + '", "' + Body.Comment + '", "' + time + '", "' + Body.Date + '", "' + Body.Time + '");';
